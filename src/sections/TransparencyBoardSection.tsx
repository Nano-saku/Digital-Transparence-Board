...
import {
  ...
  FileText, Download, MessageCircle, Receipt,
  Search, Plus, Pencil, Trash2, Loader2, Save
} from 'lucide-react';
import type { ViewState, Transaction, EventAllocation, FinancialSummary, UserRole, Event } from '@/types';
import {
  transactionsService,
  financialReportingService,
  eventsService,
} from '@/services/db';
import { autoCreateReceipt, officialReceiptNumber, downloadBlob } from '@/lib/receipts';
...
export default function TransparencyBoardSection({ adminMode = false, onBack, onNavigate, role, staffName = '' }: TransparencyBoardSectionProps) {
  ...
  const canManageLedger = Boolean(
    adminMode && (role === 'admin' || role === 'treasurer' || role === 'auditor')
  );
  ...
  useSectionEntrance(sectionRef, [
      // Headline entrance
      { ref: headlineRef, from: { x: '-40vw', opacity: 0 }, to: { x: 0, opacity: 1, duration: 0.7 } },
      // Summary cards entrance
      { ref: summaryRef, selector: '.summary-card', from: { y: '-30vh', opacity: 0 }, to: { y: 0, opacity: 1, duration: 0.6, stagger: 0.06 }, position: '-=0.4' },
      // Allocation table entrance
      { ref: allocationRef, from: { x: '-60vw', opacity: 0 }, to: { x: 0, opacity: 1, duration: 0.7 }, position: '-=0.3' },
      // Ledger entrance
      { ref: ledgerRef, from: { x: '60vw', opacity: 0 }, to: { x: 0, opacity: 1, duration: 0.7 }, position: '-=0.5' },
      // CTA row entrance
      { ref: ctaRef, from: { y: '30vh', opacity: 0 }, to: { y: 0, opacity: 1, duration: 0.6 }, position: '-=0.3' },
    ]);
    ...
    try {
      ...
      const escaped = (value: string) =>
        String(value)
          ...
          .replace(/>/g, "&gt;")
          .replace(/"/g, "&quot;");
      const allocationRows = eventAllocations.map(
          (a) => `<tr>
...
<body>
  ...
  <div class="cards">
    <div class="card"><div class="label">Total Budget</div><div class="value">${formatPeso(financialSummary?.totalBudget ?? 0)}</div></div>
  ...
  <footer>Generated by the Digital Transparency Board � for questions contact the Local Student Council treasurer or auditor.</footer>
      ...
      toast.success("Report downloaded. Open it in a browser and use Print > Save as PDF to export.");
    } catch (error) {
  ...
  const handleSaveTransaction = async () => {
    ...
    try {
      setSaving(true);
      const eventName = events.find((e) => e.id === transactionForm.eventId)?.name;
      ...
      if (!receiptUrl && (role === 'admin' || role === 'treasurer' || role === 'auditor')) {
        try {
  ...
  const handleDeleteTransaction = async (transaction: Transaction) => {
    if (!window.confirm(`Delete transaction "${transaction.description}"? This cannot be undone.`)) return;
    try {
          ...
          <>
            ...
            <div ref={allocationRef} className="glass-card p-5 lg:p-6 mb-8">
                  ...
                  <tbody>
                    {eventAllocations.map((allocation) => (
                      <tr key={allocation.eventId}>
                        ...
                        <td className="text-text-secondary">{formatPeso(allocation.allocationAmount)}</td>
                        <td className="text-green-600">{formatPeso(allocation.totalCollected)}</td>
                        <td className="text-red">{formatPeso(allocation.totalSpent)}</td>
                        <td className="font-medium text-dark">{formatPeso(allocation.remainingBalance)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              {eventAllocations.length === 0 && (
                <SectionEmptyState message="No event allocations found" icon={PieChart} compact />
              )}
            </div>
...
onClick={() => handleDeleteTransaction(transaction)}
              ...
              {filteredTransactions.length === 0 && (
                <SectionEmptyState message="No transactions found" icon={FileText} compact />
              )}
            </div>
      ...
      >
        <DialogContent className="glass-card-strong max-w-md">
          ...
          <div className="space-y-4 mt-4">
            ...
            <div>
              <label className="block text-sm font-medium text-dark mb-1">Event (optional)</label>
              <select
                value={transactionForm.eventId}
                onChange={(e) => setTransactionForm({ ...transactionForm, eventId: e.target.value })}
