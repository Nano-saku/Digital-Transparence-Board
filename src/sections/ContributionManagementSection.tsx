...
import {
  Search, Plus, Edit2, Trash2, User, Calendar, Save, Loader2, Coins, FileText
} from 'lucide-react';
import { useSectionEntrance } from '@/hooks/useSectionEntrance';
import SectionLoader from '@/components/SectionLoader';
import SectionEmptyState from '@/components/SectionEmptyState';
import SectionBackButton from '@/components/SectionBackButton';
import { contributionsService, studentsService, eventsService, subscribeToTables } from '@/services/db';
...
import { parseCsv, excelRowsToRecords, pickField, parseAmount } from '@/lib/spreadsheet';
...
export default function ContributionManagementSection({ onBack }: ContributionManagementSectionProps) {
...
useSectionEntrance(sectionRef, [
  ...
  // Summary stats
  const totalRequired = useMemo(() => records.reduce((sum, r) => sum + r.requiredAmount, 0), [records]);
  const totalPaid = useMemo(() => records.reduce((sum, r) => sum + r.amountPaid, 0), [records]);
  const totalBalance = useMemo(() => records.reduce((sum, r) => sum + r.remainingBalance, 0), [records]);
  ...
  const handleSave = async () => {
    ...
    if (conflict) {
      toast.error('This student already has a contribution record for that event');
      return;
    }
    ...
    try {
      ...
      if (editingRecord) {
        const updated = await contributionsService.update(editingRecord.id, payload);
        setRecords(records.map((r) => (r.id === editingRecord.id ? enrich(updated) : r)));
  ...
  const handleDelete = async (record: ContributionRow) => {
    if (!confirm(`Delete ${record.studentName}'s contribution for ${record.eventName}?`)) return;
    try {
      await contributionsService.delete(record.id);
      setRecords(records.filter((r) => r.id !== record.id));
      toast.success('Contribution record deleted');
    } catch (error) {
      console.error('Error deleting contribution record:', error);
  ...
  const importContributionRows = async (rows: Record<string, string>[]) => {
    ...
    const studentById = new Map(students.map((s) => [s.studentId.toLowerCase(), s]));
    ...
    for (const row of rows) {
      const fileStudentId = pickField(row, 'studentid', 'studentno', 'studentnumber', 'studid', 'idnumber', 'lrn', 'id');
      const studentName = pickField(row, 'name', 'fullname', 'studentname');
      const eventName = pickField(row, 'event', 'eventname', 'activity', 'contributionfor');

      const student =
        studentById.get(fileStudentId.toLowerCase()) ?? studentByName.get(studentName.toLowerCase());
      ...
      const requiredAmount = parseAmount(
        pickField(row, 'required', 'requiredamount', 'requiredamountpeso', 'amount', 'fee', 'contribution')
      );
      if (requiredAmount <= 0) {
        unmatched++;
        continue;
      ...
      const amountPaid = parseAmount(pickField(row, 'amountpaid', 'paid', 'paidamount', 'collected', 'payment'));
    ...
    if (parsed.length === 0) {
      toast.error('No valid rows found. Expected columns: Student ID / Name, Event, Required Amount, Amount Paid');
      return;
    }

    // Skip rows that already exist in the table or are duplicated in the file.
    const existingKeys = new Set(records.map((r) => `${r.studentId.toLowerCase()}|${r.eventId.toLowerCase()}`));
...
onClick={openAddModal}
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
          <div className="glass-card px-4 py-3">
            ...
            <p className="text-xl font-bold text-dark">{records.length.toLocaleString()}</p>
          </div>
          <div className="glass-card px-4 py-3">
            <p className="text-xs text-text-secondary">Total Required</p>
            <p className="text-xl font-bold text-purple-600">{formatPeso(totalRequired)}</p>
          </div>
          <div className="glass-card px-4 py-3">
            <p className="text-xs text-text-secondary">Total Collected</p>
            <p className="text-xl font-bold text-green-600">{formatPeso(totalPaid)}</p>
          </div>
          <div className="glass-card px-4 py-3">
            <p className="text-xs text-text-secondary">Total Balance</p>
            <p className="text-xl font-bold text-red">{formatPeso(totalBalance)}</p>
          </div>
        </div>
                      ...
                      <tr key={record.id} className="group">
                        <td>
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              <span className="font-medium text-dark block">{record.studentName}</span>
                              <span className="text-xs text-text-secondary">{record.studentId}</span>
                            </div>
                          </div>
                        </td>
                        <td>
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-text-secondary">{record.eventName}</span>
                          </div>
                        </td>
                        <td className="text-right text-text-secondary">{formatPeso(record.requiredAmount)}</td>
                        <td className="text-right font-medium text-green-600">{formatPeso(record.amountPaid)}</td>
                        <td className="text-right text-text-secondary">{formatPeso(record.remainingBalance)}</td>
                        <td className={`font-medium ${status.className}`}>{status.label}</td>
...
onClick={() => handleDelete(record)}
            ...
            {filteredRecords.length === 0 && (
              <SectionEmptyState message="No contribution records found" icon={Coins} compact />
            )}
          </div>
        )}
        ...
        <div className="mt-4 flex flex-wrap gap-4 text-sm text-text-secondary">
          <span>Total Records: <strong className="text-dark">{records.length}</strong></span>
          <span>Filtered: <strong className="text-dark">{filteredRecords.length}</strong></span>
        </div>
      </div>
      ...
      >
        <DialogContent className="glass-card-strong max-w-lg">
          ...
          <div className="space-y-4 mt-4">
            <div>
              <label className="block text-sm font-medium text-dark mb-1">Student</label>
              <select
            ...
            <div>
              <label className="block text-sm font-medium text-dark mb-1">Event</label>
              <select
                value={form.eventId}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-dark mb-1">Required Amount (₱)</label>
                <input
                  type="number"
                  min={0}
                  step={0.01}
                  value={form.requiredAmount || ''}
                  onChange={(e) => setForm({ ...form, requiredAmount: Number(e.target.value) })}
                  className="glass-input w-full px-4 py-2"
                  placeholder="e.g., 150"
                  disabled={saving}
                />
            ...
            <div className="rounded-xl bg-white/30 border border-white/50 px-4 py-3 flex items-center justify-between text-sm">
              ...
              <span className={`font-semibold ${computedBalance > 0 ? 'text-red-500' : 'text-green-600'}`}>
...
onClick={handleSave}
                ...
                disabled={saving || !form.studentId || !form.eventId || form.requiredAmount <= 0}
              >
                {saving ? (
                  <>
                    <Loader2 className="w-4 h-4 animate-spin" />
